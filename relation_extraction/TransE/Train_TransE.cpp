             	...
             	for (int batch = 0; batch<nbatches; batch++)
             	{
            		...
            		entity_tmp = entity_vec;
             		for (int k=0; k<batchsize; k++)
             		{
						...
						double pr = 1000*right_num[fb_l[i]]/(right_num[fb_l[i]]+left_num[fb_l[i]]);
						if (method ==0)
                            pr = 500;
						if (rand()%1000<pr)
						{
							while (ok[make_pair(fb_h[i],fb_l[i])].count(j)>0)
								j=rand_max(entity_num);
							train_kb(fb_h[i],fb_t[i],fb_l[i],fb_h[i],j,fb_l[i]);
						}
						else
						{
							while (ok[make_pair(j,fb_l[i])].count(fb_t[i])>0)
								j=rand_max(entity_num);
							train_kb(fb_h[i],fb_t[i],fb_l[i],j,fb_t[i],fb_l[i]);
						}
                		norm(relation_tmp[fb_l[i]]);
...
void prepare()
{
	...
	for(int i=0; i<50; i++)
		printf("%.4f\t", left_num[i]);
	printf("\n");
	for(int i=0; i<50; i++)
		printf("%.4f\t", right_num[i]);
	printf("\n");
    fclose(f_kb);
}
